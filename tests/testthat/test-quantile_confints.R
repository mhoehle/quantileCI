...
if (FALSE) {
  ...
  (ci <- quantileCI::quantile_confint_nyblom(x, p=p, conf.level=conf.level, interpolate=FALSE))
  (idx <- pmatch(ci, x))
  ...
  quantileCI::quantile_confint_nyblom(x, p=p, conf.level=0.9)
  ...
  simulate.coverage_qci <- function(n=n,p=p,conf.level=0.9, nSim=25e3, ...) {
  ...
  ##(this might be slow) Try to do this using numerical integration
  ##instead.
  ...
  c(lambda1,lambda2)  ## lambda1 <- 0.09313647, ## lambda2 <- 0.4259059
  ...
  p_wsum <- function(xi_p, r,lambda, F, f) {
    ...
    f_joint <- function(xr, xrp1, r) {
      (xrp1 > xr) * exp(lfactorial(n) - lfactorial(r-1) - lfactorial(n - r -1)) * F(xr)^(r-1) * (1-F(xrp1))^(n-r-1)*f(xr)*f(xrp1)
    }

    grid <- expand.grid(xr=xr <- seq(-50,20,length=1000),xrp1=xr)
    y <- f_joint(grid$xr, grid$xrp1, r=d)
    image(xr,xr,matrix(log(y), ncol=length(xr),nrow=length(xr)))
    xr <- seq(-40,20,length=100)
    z <- x_p
    xrp1 <- (z - (1-lambda)*xr)/lambda
    points(xr, xrp1,type="l",lwd=2)

    sum(f_joint(grid$xr, grid$xrp1, r=d)) * (xr[2] - xr[1])^2

    sum(f_joint(xr, grid$xrp1, r=d)) * (xr[2] - xr[1])^2
    ...
    f_z <- function(z) {
      ...
      f_to_int <- function(xr,z) {
        xrp1 <- (z - (1-lambda)*xr)/lambda
        if (!(isTRUE(all.equal((1-lambda)*xr + (lambda)*xrp1, rep(z,length(xr)))))) { #cons. check
          browser()
        }
        if (!all((xrp1 > xr))) {  browser()}
        f_joint(xr, xrp1, r)
      }
      ##Density PDF of z = (1-lambda)*xr + (lambda)*xrp1
      fz_one <- function(z) {
        integrate( f_to_int, lower=-Inf, upper=z, z=z,abs.tol=1e-12)$value
      }

      ## print(z)
      ## xr_grid <- seq(-10,z[2],length=1000)
      ## plot(xr_grid, sapply(xr_grid, fZ),type="l")

      sapply(z, fz_one)
    }


    z_grid <- seq(-15,15,length=1000)
    plot(z_grid, sapply(z_grid, f_z),type="l")
    integrate(f_z, lower=-5,upper=Inf,abs.tol=1e-12)$value

    #Calculate P(z >= x_p) by numerical integration
    integrate(f_z, lower=x_p,upper=Inf,abs.tol=1e-12)$value
  }
