...
quantile_confint_so <-function(x,q,conf.level=0.95, x_is_sorted=FALSE){
# x is the sample, p (0<p<1) the quantile, P the confidence level
  if (!x_is_sorted) {x<-sort(x)}
  alpha <- 1-conf.level
  n<-length(x)
  s <- min(which(pbinom((0:n),n,q) >= 1-(alpha)/2))
  r <- max(which(pbinom((0:n),n,q) <= (alpha)/2))
...
for (q in seq(0,1,length=10)) {
  test_that("Manual summing up probs matching qbinom quantile",
            expect_equal(quantile_confint_so(x=x, q=q, conf.level=0.95),
                         quantile_confint_nyblom(x=x, q=q, conf.level=0.95, interpolate=FALSE)))
}
